...
# Settings for input maps
briggs_weighting_list = [-1, 0, 1]
configuration_list = ['AA', 'AA4']
map_number_list = np.arange(1, 201)

# Create input identifiers
input = np.array([])
for briggs_weighting in briggs_weighting_list:
    for configuration in configuration_list:
        for map_number in map_number_list:
            # Create a unique identifier for each combination
            identifier = f'{briggs_weighting}_{configuration}_{map_number}'
            input = np.append(input, identifier)

# Define the task function
def task(identifier):
    briggs_weighting, configuration, map_number = identifier.split('_')
    briggs_weighting = int(briggs_weighting)
